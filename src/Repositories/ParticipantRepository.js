class ParticipantsRepository {
    ...
    async GetRandomParticipant(guild_id) {
        return this.dbAdapter
            .get(
                "SELECT id, discord_user_id, discord_user_name FROM participants WHERE discord_guild_id = ?1 AND excluded = 0 ORDER BY random() LIMIT 1",
                { 1: guild_id }
            )
    ...
    AddParticipant(user_id, guild_id, name) {
        this.dbAdapter.run(
            ...
            { 1: user_id, 2: guild_id, 3: name, 4: 0 }
        );
    }

    RemoveParticipant(user_id, guild_id) {
        this.dbAdapter.run(
            "UPDATE participants SET excluded = 1 WHERE discord_user_id = ?1 AND discord_guild_id = ?2",
            { 1: user_id, 2: guild_id }
        );
    }
    ...
    IsParticipantExists(user_id, guild_id) {
        return this.dbAdapter
            .get(
                "SELECT count(id) as cnt FROM participants WHERE discord_user_id = ?1 AND discord_guild_id = ?2 AND excluded = 0",
                { 1: user_id, 2: guild_id }
            )
